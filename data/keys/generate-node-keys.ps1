...
function generate_keys($node_dir) {
    ...
    docker run --rm -v ${PWD}/config/genesis.json:/data/genesis.json -v ${PWD}/config/qbftConfigFile.json:/opt/besu/qbftConfigFile.json -v ${PWD}/config:/opt/besu/config -v ${PWD}/data/nodes/${node_dir}/:/opt/besu/data hyperledger/besu:latest operator generate-blockchain-config --config-file=/opt/besu/qbftConfigFile.json --to=/opt/besu/data --private-key-file-name=key
